...
{
  *traceFile->GetStream () << AddressToString (from) << ","
...
//  wifi.SetStandard (WIFI_STANDARD_80211ac);
  ...
  {
    positionAlloc->Add (Vector (std::cos (i * dTheta) * distance,
                                std::sin (i * dTheta) * distance,
                                0.0));
  }
  ...
  uint16_t port = 50000;
  BurstSinkHelper server ("ns3::UdpSocketFactory",
  ...
  if (burstGeneratorType == "model")
  ...
  else if (burstGeneratorType == "trace")
    {
      ...
      client.SetBurstGenerator ("ns3::TraceFileBurstGenerator",
                                "TraceFile", StringValue (filenameSs.str ()));
    }
  else if (burstGeneratorType == "deterministic")
    {
      ...
      client.SetBurstGenerator ("ns3::SimpleBurstGenerator",
                                "PeriodRv", StringValue (periodSs.str ()),
                                "BurstSizeRv", StringValue (burstSizeSs.str ()));
    }
  else
    {
  ...
  *fragmentTrace->GetStream ()
  ...
  for (uint32_t i = 0; i < nStas; i++)
    {
      uint64_t burstsSent =
          DynamicCast<BurstyApplication> (clientApps.Get (i))->GetTotalTxBursts ();
