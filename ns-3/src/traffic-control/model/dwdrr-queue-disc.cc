...
NS_LOG_COMPONENT_DEFINE ("DWDRRQueueDisc");

NS_OBJECT_ENSURE_REGISTERED (DWDRRFlow);

TypeId DWDRRFlow::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::DWDRRFlow")
                          .SetParent<QueueDiscClass> ()
                          .SetGroupName ("TrafficControl")
                          .AddConstructor<DWDRRFlow> ()
      ;
  return tid;
}
...
void
DWDRRFlow::SetDeficit (uint32_t deficit)
{
...
int32_t
DWDRRFlow::GetDeficit (void) const
{
  NS_LOG_FUNCTION (this);
  return m_deficit;
}
...
void
DWDRRFlow::IncreaseDeficit (int32_t deficit)
{
  NS_LOG_FUNCTION (this << deficit);
  m_deficit += deficit;
}
...
void
DWDRRFlow::SetStatus (FlowStatus status)
{
  NS_LOG_FUNCTION (this);
  m_status = status;
}

DWDRRFlow::FlowStatus
DWDRRFlow::GetStatus (void) const
{
  NS_LOG_FUNCTION (this);
  return m_status;
}
...
NS_OBJECT_ENSURE_REGISTERED (DWDRRQueueDisc);

TypeId DWDRRQueueDisc::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::DWDRRQueueDisc")
                          .SetParent<QueueDisc> ()
                          .SetGroupName ("TrafficControl")
                          .AddConstructor<DWDRRQueueDisc> ()
                          .AddAttribute ("ByteSoftLimit",
                                         "The soft limit on the real queue size, measured in bytes",
                                         UintegerValue (80 * 1024),
                                         MakeUintegerAccessor (&DWDRRQueueDisc::m_soft_limit),
                                         MakeUintegerChecker<uint32_t> ())
                          .AddAttribute ("ByteHardLimit",
                                         "The hard limit on the real queue size, measured in bytes",
                                         UintegerValue (100 * 1024),
                                         MakeUintegerAccessor (&DWDRRQueueDisc::m_hard_limit),
                                         MakeUintegerChecker<uint32_t> ())
                          .AddAttribute ("Flows",
                                         "The number of queues into which the incoming packets are classified",
                                         UintegerValue (1024),
                                         MakeUintegerAccessor (&DWDRRQueueDisc::m_flows),
                                         MakeUintegerChecker<uint32_t> ())
      ;
  return tid;
}

DWDRRQueueDisc::DWDRRQueueDisc ()
    : m_quantum (0)
{
  NS_LOG_FUNCTION (this);
}
...
void
DWDRRQueueDisc::SetQuantum (uint32_t quantum)
{
...
uint32_t
DWDRRQueueDisc::GetQuantum (void) const
{
  return m_quantum;
}

bool
DWDRRQueueDisc::DoEnqueue (Ptr<QueueDiscItem> item)
{
  ...
  if (m_flowsIndices.find (h) == m_flowsIndices.end ())
    {
      ...
      flow = m_flowFactory.Create<DWDRRFlow> ();
    ...
    {
      flow = StaticCast<DWDRRFlow> (GetQueueDiscClass (m_flowsIndices[h]));
    }
  ...
  if (flow->GetStatus () == DWDRRFlow::INACTIVE)
    {
      NS_LOG_DEBUG ("Setting flow as ACTIVE");
      flow->SetStatus (DWDRRFlow::ACTIVE);
              ...
              if (flow->GetQueueDisc ()->GetNPackets () == 0)
                {
                  ...
                  flow->SetDeficit (0);
                  flow->SetStatus (DWDRRFlow::INACTIVE);
                }
...
DWDRRQueueDisc::DoPeek (void) const
{
  NS_LOG_FUNCTION (this);
...
bool
DWDRRQueueDisc::CheckConfig (void)
{
  NS_LOG_FUNCTION (this);
  if (GetNQueueDiscClasses () > 0)
    {
      NS_LOG_ERROR ("DWDRRQueueDisc cannot have classes");
      return false;
    }
  ...
  if (GetNPacketFilters () == 0)
    {
      ...
      //      NS_LOG_ERROR ("DWDRRQueueDisc needs at least a packet filter");
      //      return false;
    }

  if (GetNInternalQueues () > 0)
    {
      NS_LOG_ERROR ("DWDRRQueueDisc cannot have internal queues");
      return false;
    }
...
void
DWDRRQueueDisc::InitializeParams (void)
{
  NS_LOG_FUNCTION (this);
  ...
  m_flowFactory.SetTypeId ("ns3::DWDRRFlow");
